$(function() {
	...
	option = $("<option>").val("0").text("==请选择农场==");
    $("[name='farm']").append(option);
        ...
        if (code > 0) {
            for (var i = 0; i < areas.length; i++) {
                if (parseInt(areas[i].code) == code) {
                    ...
                    loadPlace(longitude, latitude, 8);
        ...
        if (code > 0) {
            for (var i = 0; i < areas.length; i++) {
                if (parseInt(areas[i].code) == code) {
                    ...
                    loadPlace(longitude, latitude, 11);
        ...
        if (code > 0) {
            for (var i = 0; i < areas.length; i++) {
                if (parseInt(areas[i].code) == code) {
                    ...
                    loadPlace(longitude, latitude, 14);
    ...
    //农场联动
    $("[name='farm']").bind("change", function() {
        var code = parseInt($(this).val());
        ...
        if (code > 0) {
            for (var i = 0; i < areas.length; i++) {
                if (parseInt(areas[i].code) == code) {
                    ...
                    loadPlace(longitude, latitude,17);
